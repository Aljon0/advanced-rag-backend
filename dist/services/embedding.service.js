...
async function embedBatch(chunks, batchNumber, totalBatches) {
    console.log(`[Embedding] Processing batch ${batchNumber}/${totalBatches} ` +
    ...
    try {
        ...
        if (embeddings.length !== chunks.length) {
            throw new AppError(`Embedding count mismatch: expected ${chunks.length}, got ${embeddings.length}`, 500, "EMBEDDING_MISMATCH");
        }
        ...
        if (err instanceof AppError)
            throw err;
        throw new AppError(`Failed to generate embeddings for batch ${batchNumber}: ${err instanceof Error ? err.message : "Unknown error"}`, 500, "EMBEDDING_FAILED");
    }
}
// ─── Main: Embed All Chunks in Batches ───────────────────────────────────────
export async function embedChunks(chunks) {
    if (chunks.length === 0) {
        throw new AppError("No chunks provided for embedding.", 400, "NO_CHUNKS");
    }
    ...
    console.log(`[Embedding] Starting embedding of ${chunks.length} chunks ` +
    ...
    if (actualDimension !== expectedDimension) {
        throw new AppError(`Embedding dimension mismatch: expected ${expectedDimension}, ` +
            `got ${actualDimension}. Check your Supabase vector column size.`, 500, "EMBEDDING_DIMENSION_MISMATCH");
    }
    return embeddedChunks;
}
// ─── Stats Helper ─────────────────────────────────────────────────────────────
export function getEmbeddingStats(chunks) {
    return {
        total: chunks.length,
