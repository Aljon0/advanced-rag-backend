...
export function validatePdfFile(file) {
    if (!ALLOWED_MIME_TYPES.includes(file.mimetype)) {
        throw new AppError("Invalid file type. Only PDF files are allowed.", 400, "INVALID_FILE_TYPE");
    }
...
export async function extractTextFromPdf(buffer) {
    ...
    try {
        ...
        if (!textResult.text || textResult.text.trim().length === 0) {
            throw new AppError("Could not extract text from PDF. The file may be scanned or image-based.", 422, "PDF_EXTRACTION_FAILED");
        }
        ...
        if (err instanceof AppError)
            throw err;
        throw new AppError("Failed to parse PDF file. Please ensure the file is not corrupted.", 422, "PDF_PARSE_ERROR");
    }
    finally {
