...
function parseMistralResponse(raw) {
    try {
        ...
        return {
            ...
            citedChunks: Array.isArray(parsed.citedChunks)
                ? parsed.citedChunks.filter((i) => typeof i === "number")
                : [],
        };
    }
    catch {
...
// ─── Build Citations ──────────────────────────────────────────────────────────
function buildCitations(citedChunkIndices, chunks) {
    const seen = new Set();
...
// ─── Main: RAG Pipeline ───────────────────────────────────────────────────────
export async function ragAnswer(question, sessionId) {
    ...
    console.log(`[RAG] Using ${contextChunks.length} chunks as context`);
    ...
    try {
        ...
        throw new AppError(`AI generation failed: ${err instanceof Error ? err.message : "Unknown error"}`, 500, "AI_GENERATION_FAILED");
    }
