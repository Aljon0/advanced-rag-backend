...
function chunkPageText(text, pageNumber, startingChunkIndex) {
    ...
    for (let i = 0; i < sentences.length; i++) {
        ...
        if (currentChunk.length + sentence.length > CHUNK_SIZE &&
            ...
            for (let j = i - 1; j >= 0; j--) {
                const s = sentences[j];
                if (overlapLength + s.length > CHUNK_OVERLAP)
                    break;
...
export function chunkExtractedPages(pages) {
    ...
    for (const page of pages) {
        ...
        const pageChunks = chunkPageText(page.content, page.pageNumber, globalChunkIndex);
        allChunks.push(...pageChunks);
        globalChunkIndex += pageChunks.length;
    }
    console.log(`[Chunking] Created ${allChunks.length} chunks ` +
...
export function getChunkingStats(chunks) {
    ...
    return {
        total: chunks.length,
        avgCharacters: Math.round(lengths.reduce((a, b) => a + b, 0) / lengths.length),
